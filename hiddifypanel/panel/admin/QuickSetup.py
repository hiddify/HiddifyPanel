...
class QuickSetup(FlaskView):
    ...
    def index(self):
        return render_template('quick_setup.html', lang_form=get_lang_form(), form=get_quick_setup_form(), ipv4=ip_utils.get_ip(4), ipv6=ip_utils.get_ip(6), admin_link=admin_link(), show_domain_info=True)
    ...
    def post(self):
        ...
        if lang_form.lang_submit.data:
            ...
            return render_template('quick_setup.html', form=get_quick_setup_form(True), lang_form=get_lang_form(), admin_link=admin_link(), ipv4=ip_utils.get_ip(4), ipv6=ip_utils.get_ip(6), show_domain_info=False)

        if quick_form.validate_on_submit():
            sslip_dm = Domain.query.filter(Domain.domain == f'{ip_utils.get_ip(4)}.sslip.io').delete()
        ...
        return render_template('quick_setup.html', form=quick_form, lang_form=get_lang_form(True), ipv4=ip_utils.get_ip(4), ipv6=ip_utils.get_ip(6), admin_link=admin_link(), show_domain_info=False)
...
def admin_link():
    ...
    if len(domains):
        ...
        return "https://"+ip_utils.get_ip(4)+hiddify.get_admin_path()
