...
class ShortAPI(MethodView):
    ...
    def get(self):
        ...
        short, expire_in = hiddify.add_short_link(f'https://{g.account.username}:{g.account.password}@{urlparse(request.host)}/{hconfig(ConfigEnum.proxy_path)}/#{g.account.name}')
