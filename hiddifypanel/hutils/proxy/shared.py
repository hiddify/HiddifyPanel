...
from hiddifypanel.models import Proxy, ProxyTransport, Domain, DomainType, ProxyCDN, ProxyProto, hconfig, ConfigEnum, get_hconfigs
from hiddifypanel.cache import cache
from hiddifypanel import hutils


def __is_tls(l3) -> bool:
...
def make_proxy(hconfigs: dict, proxy: Proxy, domain_db: Domain, phttp=80, ptls=443, pport: int | None = None) -> dict:
    ...
    name = proxy.name
    port = __get_port(proxy, hconfigs, domain_db, ptls, phttp, pport)

    if val_res := __check_proxy_incorrect(proxy, domain_db, port):
        # print(val_res)
        return val_res
